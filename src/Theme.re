...
let of_yojson = (json: Yojson.Safe.t) => {
  ...
  let defaultBackground = ColorTheme.getFirstOrDefault(
    ~default="#000",
    ["background", "editor.background"],
    colorTheme);

  let defaultForeground = ColorTheme.getFirstOrDefault(
    ~default="#FFF",
  ...
  let tokenTheme = TokenTheme.of_yojson(~defaultBackground, ~defaultForeground, tokenColorsJson);
