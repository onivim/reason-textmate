...
let push = (~matches: array(OnigRegExp.Match.t), ~matchRange: Pattern.matchRange, ~line: int, v: t) => {
  ignore(line);

  let newMatchRange = if(RegExp.hasBackReferences(matchRange.endRegex)) {
    // If the end range has back references, we need to resolve them from the provided matches
    
    let matchGroups = matches
    |> Array.to_list
    ...
    let resolvedEndRegex = RegExp.supplyReferences(matchGroups, matchRange.endRegex);
    {
      ...matchRange,
